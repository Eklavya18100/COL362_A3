...
// #define MAXSIZE 15728640 // 800ULL * 1024ULL * 1024ULL
const uint64_t maxsize = 80ULL * 1024ULL * 1024ULL; // 800MB
...
void createInitialRuns(const string &inputF, const long key_count)
{
    ...
    while (getline(inputFile, line) && keys < key_count)
    {
        ...
        while (bytes_written < maxsize && getline(inputFile, line) && keys < key_count)
        {
        ...
        while (t-- != 0)
        {
            ...
            // bytes_written += temp.front().size() + 1;
            temp.pop_front(); 
        }
        bytes_written = 0;
    }
...
int main()
{
    ...
    int totalMerges = external_merge_sort_withstop("random.list", "output", 2530475, 8);
    ...
    // int totalMerges = external_merge_sort_withstop("random3.list", "output", 100000000, 16);
    // int totalMerges = external_merge_sort_withstop("random3.list", "output", 5060950, 16);
    // int totalMerges = external_merge_sort_withstop("random3.list", "output", 50609500, 16);
    // int totalMerges = external_merge_sort_withstop("random.txt", "output", 100000000, 16);
    cout << totalMerges;
